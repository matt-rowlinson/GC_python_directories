...
def main():
    ...
    versions = ['14.0.1']
    year     = '2017'
    variables = ['C2H6','C3H8','TOLU','BENZ','ALK4','PRPE']
    
    for variable in variables:
        print( variable )
        ds = read_model_output( rundirs, versions, variable, years=year)
        df = load_observations(site, variable, year)
        
        f, ax = plt.subplots(1,1,figsize=(10,4))
        df.resample('D').mean().plot(color='k', label='Observations', ax=ax)
        labels=['Observations','Base','Geo only','Scale only','Scale and Geo']
        cs = ['#377eb8','#4daf4a','#984ea3','#ff7f00']

        for n,nn in enumerate(ds):
            ds[n].plot( label=labels[n], c=cs[n], ax=ax )

        ax.set_ylabel( f'{variable} (pptv)' )
        ax.set_xlabel( None )
        plt.legend(labels)
        plt.tight_layout()
        plt.savefig(f'plots/{site["save_name"]}.{variable}.png')
        plt.close()
