...
def main():
    ...
    sname='gridboxes'
    variables=['all-nitrate-plus-HNO3']
    years='2018'
    cs=['#1b9e77','#e7298a','#d95f02','#7570b3','#e7298a','#66a61e','#e6ab02']
    for v in variables:
        ...
        ds0 = get_data_as_xr('nitrate_photol_control', year='2017', variable=v, version='13.1.2')  
        ds0 = site_data( ds0, ds, lon=site['longitude'], lat=site['latitude'] )
        ...
        ds1 = site_data( ds1, ds, lon=site['longitude'], lat=site['latitude'] )
        ds1 = pd.DataFrame({site['save_name']:ds1.values}, index=ds1['time'].values)
        ...
        sys.path.append('/users/mjr583/cvao')
        import read_nitrate
        df = read_nitrate.read_nitrate_dataset(convert=True)['2011']
        ...
        ax.plot( dsn.index, ds1[site["save_name"]], label=versions[1], c=cs[1], zorder=2, ls=style[0])

        plt.ylabel(f'{v} ppbv')
        plt.legend(loc=0, ncol=3)
        plt.tight_layout()
        plt.savefig( f"plots/DUST_noDUST_cvao_nitrate-nitric_2011" )
        plt.close()
