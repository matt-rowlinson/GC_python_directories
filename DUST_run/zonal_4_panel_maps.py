...
def main():
    ...
    rundirs  = ['nitrate_photol_2015_control', 'nitrate_photol_control']
    versions = ['13.1.2','13.1.2_DUST']

    variables=['NITA','SO4A','NIT', 'NITs','SO4','all_nitrate','all_sulphate','O3','CO']
    ...
    for nn, v in enumerate(variables):
        ...
        plot( f, axes, add_plottable, lat, lon, c=cmap, 
                cbar_label=[f'{units[nn]}',f'{units[nn]}',f'{units[nn]}','%'],
                labels=['v13.1.2', 'v13.1.2_Dust', 'Dust - Base','Dust / Base'],
                sname=f'DUSTY_DIFF_{v}')
