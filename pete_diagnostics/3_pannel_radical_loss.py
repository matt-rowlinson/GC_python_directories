...
def rate_ratios(path, day, T, NUM_DEN, gamma, latlon=None):
    with xr.open_dataset(f"{path}/OutputDir/GEOSChem.ConcAfterChem.{day}.nc4") as data: 
        ...
        ROX = 0
        #ROx + ROx
        with open("/users/mjr583/GC/pete_diagnostics/ROx_self_reactions.txt") as file:
            ...
            for i,L in enumerate(LINES):
                ...
                elif REACTION[0] == "GCJPLPR":
                    RATE = GCJPLPR(float(REACTION[1]), float(REACTION[2]), float(REACTION[3]), float(REACTION[4]), float(REACTION[5]), float(REACTION[6]), float(REACTION[7]), T, NUM_DEN) * A * B
                elif REACTION[0] == "GC_HO2HO2":
                    RATE = GC_HO2HO2(float(REACTION[1]), float(REACTION[2]), float(REACTION[3]), float(REACTION[4]), float(REACTION[5]), float(REACTION[6]), T, H2O, NUM_DEN) * A * B
                elif REACTION[0] == "GC_TBRANCH":
                    RATE = GC_TBRANCH(float(REACTION[1]), float(REACTION[2]), float(REACTION[3]), float(REACTION[4]), float(REACTION[5]), float(REACTION[6]), T) * A * B
                elif REACTION[0] == "GC_RO2HO2":
                    RATE = GC_RO2HO2(float(REACTION[1]), float(REACTION[2]), float(REACTION[3]), float(REACTION[4]), float(REACTION[5]), float(REACTION[6]), T) * A * B
                elif REACTION[0] == "ARRPLUS":
                    RATE = ARRPLUS(float(REACTION[1]), float(REACTION[2]), float(REACTION[3]), float(REACTION[4]), float(REACTION[5]), T) * A * B
                else:
...
def RGB_plot(latlon=None, name="", n_bins=7):
    ...
    #RGB_plot_year(ax1, f"../1750000/geosfp_0.5x0.625_tropchem", "1750", n_bins, latlon=latlon)
    RGB_plot_year(ax1, f"/mnt/lustre/users/mjr583/GC/13.1.2_diagnostics/rundirs/test/", "2019", n_bins, latlon=latlon)

    RGB_plot_year(ax2, f"../1970/geosfp_0.5x0.625_tropchem", "1970", n_bins, latlon=latlon)
    RGB_plot_year(ax3, f"../2017/geosfp_0.5x0.625_tropchem", "2014", n_bins, latlon=latlon)
    ...
    if name != "":
        name = "_"+name    
    plt.savefig(f"RGB_plots/RGB_{name}_noon.png".replace(" ","_"), dpi=300, bbox_inches="tight")
    plt.close()
...
def RGB_4_plot_dif(year, run1, run2, run3, name, gamma=0.2):
    ...
    plt.savefig(f"RGB_plots/RGB_{year}_{name}.png".replace(" ","_"), dpi=300, bbox_inches="tight")
    plt.close()
...
def regime_plot(latlon=None, name=""):
    ...
    RGB_plot_year(ax1, f"../1750/geosfp_0.5x0.625_tropchem", "1750", n_bins=2, latlon=latlon)
    ...
    if name != "":
        name = "_"+name
    plt.savefig(f"RGB_plots/Regime{name}_noon.png".replace(" ","_"), dpi=300, bbox_inches="tight")
    plt.close()
...
def regime_plot_dif(year, runs, name, gamma_list=None):
    ...
    plt.savefig(f"RGB_plots/Regime_{year}{name}.png".replace(" ","_"), dpi=300, bbox_inches="tight")
    plt.close() 
