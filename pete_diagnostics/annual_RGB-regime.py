...
def calculate_day(path, M, D, gamma, latlon):
    #Day
    day = f"2019{M:02}{D:02}_0000z"
    print(day)
...
def RGB_plot_year(ax, path, year, n_bins, name="", gamma=0.2, latlon=None):
    ...
    for M in range(1,13):
        for D in range(1, days[M-1]+1):
            data, LAT, LON = calculate_day(path, M, D, gamma, latlon = latlon)
...
def RGB_plot(latlon=None, name="", n_bins=7):
    proj = ccrs.PlateCarree()
    fig, (ax1) = plt.subplots(nrows=1,ncols=1,figsize=(8, 8), subplot_kw=dict(projection=proj))
    ...
    print( pos )
    cax = fig.add_axes([pos[0]+0.01, pos[1]+0.03, (0.20*8)/9, 0.18])
...
def regime_plot(latlon=None, name=""):
    ...
    RGB_plot_year(ax1, f"/mnt/lustre/users/mjr583/GC/13.1.2_diagnostics/rundirs/test", "2019", n_bins=2, latlon=latlon)
    ...
    if name != "":
        name = "_"+name
    plt.savefig(f"RGB_plots/NEWRegime{name}_noon.png".replace(" ","_"), dpi=300, bbox_inches="tight")
    plt.close()
...
def regime_plot_dif(year, runs, name, gamma_list=None):
    ...
    plt.savefig(f"RGB_plots/NEWRegime_{year}{name}.png".replace(" ","_"), dpi=300, bbox_inches="tight")
    plt.close() 
