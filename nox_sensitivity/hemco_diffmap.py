...
path='/users/mjr583/scratch/GC/GEOS-Chem/rundirs/fullchem_2xNAM_ethane/OutputDir/'
flist=find_file_list(path, ['HEMCO_diagnostics.20160301'])
a = xr.open_mfdataset(flist)

path='/users/mjr583/scratch/GC/GEOS-Chem/rundirs/fullchem_2xEU_ethane/OutputDir/'
flist=find_file_list(path, ['HEMCO_diagnostics.20160301'])
...
for key in keys:
    if 'EmisC2H6' in  key:
        ...
        plt.savefig('plots/c2h6_HEMCO_%s.png' %key)
        plt.close()
