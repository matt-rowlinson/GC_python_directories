...
def main(): 
    #site='MAC'
    site = sites['CVO']
    rundirs  = ['new_base_4x5','shah_4x5','langmuir_RH_90_4x5']
    variable='NOx'
    ...
    for rundir in rundirs:
        ...
        for year in years:
            ...
            #    continue
            print( year )
            path=f'/mnt/lustre/users/mjr583/GC/14.1.0/rundirs/{rundir}/OutputDir/'
            ds0 = xr.open_mfdataset( find_file_list(path, [f'SpeciesConc.{year}']), combine='by_coords' )

            cosw = get_cosweights( ds0.lat )
            ds0 = ( ds0['SpeciesConcVV_NO']+ds0['SpeciesConcVV_NO2'] ) * 1e12 * cosw
    ...
    for n in range(len(rundirs)):
        ax.plot( range( 0,24 ), diur_[n], label=labels[n], zorder=3)
        ax.fill_between( range(0,24), _25[n], _75[n], alpha=.2, zorder=3 )
