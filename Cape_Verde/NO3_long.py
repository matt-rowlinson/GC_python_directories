...
def main():
    ...
    dsy = find_model_output_for_site(ds, ['nitrate_photol_control'], ['13.1.2'], var, years='20')[0]['2012':'2019']
    ...
    ax1.plot(df0.Value.resample('M').mean().index, df0.Value.resample('M').mean(), color="k", alpha=0.7, label="_nolegend_")
    ax1.plot(dsy.resample('M').mean().index, dsy.resample('M').mean(), color="green", alpha=0.7, label="_nolegend_")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'plots/cvao_{var}_lowess_LONG.png')
    plt.close()    
