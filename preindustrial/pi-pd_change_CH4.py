    ...
    for n in range(0,3):
        if plot_type=='abs':
            ...
            if first:
                ...
                sname+='_PC'
                unit = '%'
        levels=[710, 715, 720, 725, 730, 735, 740, 745, 750, 755, 760, 770, 780, 1200,  
        ...
        if n < 2:
            ...
            im0 = all_plottable[n].plot.imshow( x='lon',y='lat', ax=ax, transform=ccrs.PlateCarree(), levels=levels,
                   add_colorbar=False)
            first=False
        else:
...
def main():
    ...
    for s in species:
        print( s )
        rundirs  = ['base_run_1751','base_run_2010']   
        ds_ = surface_data(s, rundirs, versions)

        cart_plot( s, rundirs, ds_, plot_type='pc' , sname=f'PI-PD_{s}')
        cart_plot( s, rundirs, ds_, plot_type='abs', sname=f'PI-PD_{s}')
