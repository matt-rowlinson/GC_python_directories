...
def plot(f, axes, plottable, lat, lon, c='', labels=False,
    ...
    for n in range(len(plottable[:1])):
        print( n )
        m = rp.get_basemap(freq=60, ax=axes[n])
        im = m.pcolormesh(X, Y, plottable[n], cmap=cmap, vmax=6., vmin=2.)#ax_Max[n], vmin=-(ax_Max[n]),)
...
def main():
    ...
    rundirs  = ['nitrate_photol_2017_control', 'nitrate_photol_2015_all_scale-100']
    versions = ['13.1.2','13.1.2']

    variables=['O3']#,'HNO3','Iy','SALAAL','SALCAL','O3','CO','NITA','SO4A','NIT', 'NITs','SO4','SO2','all_nitrate','all_sulphate','NH3','NH4','NOx','NOy']
    units = ['ppbv']#,'pptv','pptv','pptv','pptv','pptv','ppbv','ppbv','pptv','pptv','pptv','pptv','pptv','pptv','pptv','pptv','ppbt','ppbv','pptv']
    for nn, v in enumerate(variables):
        ...
        plot( f, axes, add_plottable, lat, lon, c=cmap, 
                cbar_label=[f'{v} {units[nn]}','%'],
                labels=['j100 - Base','j100 / Base'],
                sname=f'NEW__j100_{v}')
