...
def main():
    ...
    ds = get_data_as_xr('ryan_low_res_CVAO', year='201706')
    rundir, variables, version, site = get_arguments()
    sys.path.append( '/users/mjr583/cvao' )
    from lowess_smoother import loess

    site = sites[site]
    rundirs  = ['ryan_low_res_CVAO']
    versions = ['13.1.2']
    sname='high-lowRES'
    variables=['O3', 'NIT','NITs','NOx','all_nitrate'][::-1]#,'NIT','NITs', 'NH3','HNO3','all_sulphate']
    years='2018'
    cs=['#7fc97f',
            '#beaed4',
            '#fdc086',
    ...
    for v in variables:
        hr = get_data_as_xr('ryan_high_res_CVAO', year='201706', variable=v, version='13.1.2')  
        hr = site_data( hr, ds, lon=site['longitude'], lat=site['latitude'] )
        hr = pd.DataFrame({site['save_name']:hr.values}, index=hr['time'].values)

        ds0 = get_data_as_xr('ryan_low_res_CVAO', year='201706', variable=v, version='13.1.2')  
        ds0 = site_data( ds0, ds, lon=site['longitude'], lat=site['latitude'] )
        ds0 = pd.DataFrame({site['save_name']:ds0.values}, index=ds0['time'].values)

        ds1 = get_data_as_xr('ryan_low_res_CVAO', year='201706', variable=v, version='13.1.2')  
        ...
        ds1 = get_data_as_xr('ryan_low_res_CVAO', year='201706', variable=v, version='13.1.2')  
        ds1 = site_data( ds1, ds, lon=site['longitude']+5, lat=site['latitude'] )
        dsW = pd.DataFrame({site['save_name']:ds1.values}, index=ds1['time'].values)
        
        add_times=[hr, ds0, dsE, dsNE, dsN, dsNW, dsW]
        versions=['0.5x0.625','CVAO','Gridbox E','Gridbox NE','Gridbox N','Gridbox NW', 'Gridbox W']
