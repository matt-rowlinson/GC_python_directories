...
def main():
    ...
    for variable in variables:
        ...
        dev, gclat,gclon,lev, dev_time = GC.get_gc_var(rundir='ceds_only',variable=variable,version=version, verbose=False, year='2016')
        dev2, gclat,gclon,lev, dev2_time = GC.get_gc_var(rundir='asia-meic_scale',variable=variable,version=version, verbose=False, year='2016')
        ...
        model3_.append( model3 )
    
    alk4_obs = np.genfromtxt('csvs/ALK4_obs.csv', delimiter=',')
    alk4_model2 = np.genfromtxt('csvs/ALK4_model2.csv', delimiter=',') *1e12
    alk4_model3= np.genfromtxt('csvs/ALK4_model3.csv', delimiter=',') *1e12
    ...
    Pdev = ax1.scatter( obs_[0], model2_[0], c='g', alpha=.3, zorder=1)
    a2, _, _, _ = np.linalg.lstsq(obs_[0][:,np.newaxis], model2_[0], rcond=None)
    ax1.plot(obs_[0], a2 * obs_[0], '-', c='darkgreen', zorder=10)
    ...
    # propane
    Pdev = ax2.scatter( obs_[1], model2_[1], c='g', alpha=.3, zorder=1)
    a2, _, _, _ = np.linalg.lstsq(obs_[1][:,np.newaxis], model2_[1], rcond=None)
    ax2.plot(obs_[1], a2 * obs_[1], '-', c='darkgreen', zorder=10)
    ...
    # ALK4
    Pdev = ax3.scatter( alk4_obs, alk4_model2, c='g', alpha=.05, zorder=1)
    a2, _, _, _ = np.linalg.lstsq(alk4_obs[:,np.newaxis], alk4_model2, rcond=None)
    ax3.plot(alk4_obs, a2 * alk4_obs, '-', c='darkgreen', zorder=10)
    ax3.set_ylabel(f'Model ALK4 (ppt)')
    ax3.set_xlabel(f'Observed ALK4 (ppt)')

    #ax3.set_xlim(0. , np.max([obs_[1].max(), model2_[1].max()]))
    #ax3.set_ylim(0. , np.max([obs_[1].max(), model2_[1].max()]))
