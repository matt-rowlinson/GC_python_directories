...
def main():
    ...
    ethane_all    = pd.read_csv(f'v{version}_csvs/ethane_all_scale_{version}_2016.csv').values
    propane_all   = pd.read_csv(f'v{version}_csvs/propane_all_scale_{version}_2016.csv').values 
    ...
    alk4_all  = pd.read_csv(f'v{version}_csvs/ALK4_all_scale_{version}_DLim.csv', delimiter=',')#.values *1e12
    ...
    NHethane_all    = pd.read_csv(f'v{version}_csvs/NHethane_all_scale_{version}_2016.csv').values
    NHpropane_all   = pd.read_csv(f'v{version}_csvs/NHpropane_all_scale_{version}_2016.csv').values  
    ...
    NHalk4_all  = pd.read_csv(f'v{version}_csvs/NHALK4_all_scale_{version}_DLim.csv', delimiter=',')#.values *1e12
    NHalk4_all  = NHalk4_all[(B_alk4_obs > 0).all(1)].values
    ...
    SHethane_all    = pd.read_csv(f'v{version}_csvs/SHethane_all_scale_{version}_2016.csv').values
    SHpropane_all   = pd.read_csv(f'v{version}_csvs/SHpropane_all_scale_{version}_2016.csv').values  
    ...
    SHalk4_all  = pd.read_csv(f'v{version}_csvs/SHALK4_all_scale_{version}_DLim.csv', delimiter=',')#.values *1e12
    SHalk4_all  = SHalk4_all[(C_alk4_obs > 0).all(1)].values
    
    #a,b = get_odr( ethane_obs, ethane_ceds)
    #sys.exit()
    try:
        ...
        y2_plots= [ ethane_geo, propane_geo, alk4_geo, 
    ...
    y3_plots= [ ethane_all, propane_all, alk4_all, 
    ...
    for n in range(len(x_plots)):
            Pdev  = axes[n].scatter(    x_plots[n], y1_plots[n], c=cs[0], alpha=.2, zorder=1, label='Base (no geo)')
            x_fit1, fit1 = get_odr( x_plots[n], y1_plots[n] )
            axes[n].plot(10**x_fit1, 10**fit1, c='darkblue', lw=2, label='__best fit curve')
            
            try:
                Pdev2 = axes[n].scatter(    x_plots[n], y2_plots[n], c=cs[1], marker='o', alpha=.5, zorder=2, 
                                        label='Geological emissions' )
                x_fit2, fit2 = get_odr( x_plots[n], y2_plots[n] )
                axes[n].plot(10**x_fit2, 10**fit2, c='yellow',   lw=2, label='__best fit curve')
            except:
                pass            
    ...
    plt.tight_layout()
    plt.savefig(f'plots/geo-base-scale_figure_{version}.png')
    plt.close()
