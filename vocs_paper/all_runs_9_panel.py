...
def main():
    ...
    ethane_scale  = pd.read_csv('VOC_csvs/ethane_scale_only_13.4.0_2016.csv').values
    propane_scale = pd.read_csv('VOC_csvs/propane_scale_only_13.4.0_2016.csv').values
    ...
    alk4_all= pd.read_csv('csvs/ALK4_MEIC_v13.4.DLim.csv', delimiter=',')
    alk4_all = alk4_all[(A_alk4_obs > 0).all(1)].values #* 1e12
    alk4_geo  = alk4_ceds
    alk4_scale= alk4_ceds
    ...
    NHethane_scale  = pd.read_csv('VOC_csvs/NHethane_scale_only_13.4.0_2016.csv').values
    NHpropane_scale = pd.read_csv('VOC_csvs/NHpropane_scale_only_13.4.0_2016.csv').values
    ...
    NHalk4_all= pd.read_csv('csvs/NHALK4_MEIC_v13.4.DLim.csv', delimiter=',')#.values *1e12
    NHalk4_all  = NHalk4_all[(A_alk4_obs > 0).all(1)].values #* 1e12
    NHalk4_geo  = NHalk4_ceds
    NHalk4_scale= NHalk4_ceds
    ...
    SHethane_scale  = pd.read_csv('VOC_csvs/SHethane_scale_only_13.4.0_2016.csv').values
    SHpropane_scale = pd.read_csv('VOC_csvs/SHpropane_scale_only_13.4.0_2016.csv').values
    ...
    SHalk4_ceds = pd.read_csv('VOC_csvs/SHALK4_ceds_only_v13.4.DLim.csv', delimiter=',')#.values *1e12
    ...
    SHalk4_scale= pd.read_csv('VOC_csvs/SHALK4_scale_only_v13.4.DLim.csv', delimiter=',')#.values *1e12
    SHalk4_scale  = SHalk4_scale[(A_alk4_obs > 0).all(1)].values
    ...
    y4_plots= [ ethane_all, propane_all, alk4_all, 
    ...
    for n in range(len(x_plots)):
            ...
            Pdev2 = axes[n].scatter(    x_plots[n], y2_plots[n], c=cs[1], marker='+', alpha=.5, zorder=2, label='CEDS' )
            Pdev3 = axes[n].scatter(    x_plots[n], y3_plots[n], c=cs[2], marker='+', alpha=.5, zorder=2, label='Geo' )
            Pdev4 = axes[n].scatter(    x_plots[n], y4_plots[n], c=cs[3], marker='+', alpha=.5, zorder=2, label='Both' )
            x_fit1, fit1 = get_odr( x_plots[n], y1_plots[n] )
            x_fit2, fit2 = get_odr( x_plots[n], y2_plots[n] )
            x_fit3, fit3 = get_odr( x_plots[n], y3_plots[n] )
            x_fit4, fit4 = get_odr( x_plots[n], y4_plots[n] )
            ...
            axes[n].plot(10**x_fit3, 10**fit3, c='violet', lw=2, label='__best fit curve')
            axes[n].plot(10**x_fit4, 10**fit4, c='yellow', lw=2, label='__best fit curve')
    ...
    plt.tight_layout()
    plt.savefig('plots/TEST.all_9_panel_figure_13.4.png')
    plt.close()
