...
mw_o3_kg = spec_db["O3"]["MW_g"] * 1.0e-3
...
def tropospheric_o3_burden(ds, SpConc_ds, met_ds):
    """
    Computes the tropospheric O3 burden (1e5 molec cm-3)
    ...
    Returns:
    --------
        trop_o3_burden : numpy float64
    """
    O3 = np.squeeze(SpConc_ds["SpeciesConc_O3"].values) 
    AD = np.squeeze(met_ds["Met_AD"].values)
    Tlev = np.squeeze(met_ds["Met_TropLev"].values)
   
    if O3.ndim == 4:
        O3 = np.mean(O3,0)
    
    for ilon in range(72):
        for ilat in range(46):
            for ilev in range(1,48):            
                if (ilev > Tlev[ilat,ilon]):
                    O3[ilev-1,ilat,ilon] = np.nan
     
    trop_o3_burden = np.zeros((47)) * np.nan
    for i in range(len(O3)):
            trop_o3_burden[i] = np.nansum( O3[i] * AD[i]  * mw_o3_kg/mw_air_kg  ) * 1e-9
            #x = O3[i] * (mw_o3_kg/mw_air_kg)           # convert mol mol-1 to kg kg-1
            #y = x * AD[i]                              # kg kg-1 to kg
            #trop_o3_burden[i] = np.nansum(y) * 1e-9    # sum and Kg to Tg

    trop_o3_burden=np.nansum(trop_o3_burden)
    return trop_o3_burden
...
def print_metrics(ds, SpConc_ds, met_ds, is_ch4_sim=False):
    """
    ...
    # Get tropospheric O3 burden [Tg]
    trop_o3_burden = tropospheric_o3_burden(ds, SpConc_ds, met_ds)
    ...
    msg = "CH4 lifetime w/r/t tropospheric OH     = {:.4f} years\n".format(
        ch4_life_wrt_oh)
    print(msg)
    msg = "Tropospheric O3 burden                 = {:.4f} Tg\n".format(trop_o3_burden)
    print(msg)
...
def main():
    ...
    ds = read_collection(data_dir, 'Metrics')

    # Combine all SpeciesConc output into a single Dataset
    SpConc_ds = read_collection(data_dir, 'SpeciesConc')

    # Combine all SpeciesConc output into a single Dataset
    met_ds = read_collection(data_dir, 'StateMet')
    ...
    print_metrics(ds, SpConc_ds,met_ds,  is_ch4_sim)
