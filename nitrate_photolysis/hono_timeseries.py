...
def find_model_output_for_site( rundirs, versions, v, years='2014'):
    ...
    for n, rundir in enumerate(rundirs):
        ...
        #ds0 = ds0.between_time('10:00','14:00').resample('H').mean()
        #print( ds0 )
        #sys.exit()
...
def main():
    ...
    site = sites[site]
    rundirs  = ['new_base_4x5','andersen_4x5','RH_x_andersen_4x5','rhbin_andersen_4x5','shah_4x5']
    labels = ['Base','Andersen et al.','Andersen * RH','Andersen * RH (<53%)','Shah et al. 2022']
    versions = ['14.1.0']
    variables=['HNO2', 'O3']
    years='201401'
    cs=['k','#1b9e77','#e7298a','#d95f02','#7570b3','#e7298a','#66a61e','#e6ab02','k'][::-1]
    for v in variables:
        print( v )
        ...
        ax.plot( add_times[1].index, add_times[1][site["save_name"]], label=labels[1],
                     c=cs[1], zorder=2, ls='--')
        ax.plot( add_times[2].index, add_times[2][site["save_name"]], label=labels[2],
                     c=cs[2], zorder=2, ls='--')
        ax.plot( add_times[3].index, add_times[3][site["save_name"]], label=labels[3],
                     c=cs[3], zorder=2, ls='--')
        ax.plot( add_times[4].index, add_times[4][site["save_name"]], label=labels[4],
                     c=cs[4], zorder=2, ls='--')
        ...
        plt.tight_layout()
        plt.savefig( f"plots/new.{site['save_name']}_{v}_mean.png" )
        plt.close()
