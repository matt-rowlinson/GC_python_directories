...
for ilon in range(len(Tlon)):
    for ilat in range(len(Tlat)):
        if -169.2 < Tlon[ilon] < -50.:
            if 24. < Tlat[ilat] < 70.6:
                pass
            else:
                em[ilat, ilon] = np.nan
        else:
...
m = rp.get_basemap(lllat=20, urlat=75, lllon=-170, urlon=-45, ax=ax1)
m.pcolormesh(X,Y, CEDS)
X,Y = np.meshgrid(Tlon, Tlat)
m = rp.get_basemap(lllat=20, urlat=75, lllon=-170, urlon=-45, ax=ax2)
m.pcolormesh(X,Y, Tzompa)
ax1.set_title('CEDS C2H6')
ax2.set_title('Tzompa C2H6')

plt.savefig('BOTH_USA_C2H6.png')
plt.close()
