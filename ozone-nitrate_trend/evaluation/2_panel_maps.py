...
def get_data_as_xr(rundir, year='', lev=0, variable=False):
    ...
    #ds = ds.sel( lon=slice(-40., 10.) )
    #ds = ds.sel( lat=slice(-5., 37.) )

    lat = ds['lat']#.sel( lat=slice(-5., 37.) )
    lon = ds['lon']#.sel( lon=slice(-40., 10.) )
    return ds, lat, lon
...
def main():
    ...
    rundirs = ['nitrate_photol_control', f'ryan_high_res_CVAO']
    ...
    for v in variables:
        ...
        print( 'interpolate coarse to high' )
        X, Y = np.meshgrid(lon, lat)
        f = interpolate.interp2d(X,Y,ds0,kind='cubic')
        ds2 = f(Hlon, Hlat)
        
        print( 'interpolate fine to coarse' )
        HX, HY = np.meshgrid(Hlon, Hlat)
        #
        #f = interpolate.interp2d(HX,HY,ds1,kind='cubic')
        #ds3 = f(lon, lat)
        ...
        #im = m3.pcolormesh(X, Y, ds3 - ds0, cmap=cmap )
        
        m4 = rp.get_basemap(lllon=Hlon.min(), urlon=Hlon.max(), lllat=Hlat.min(), urlat=Hlat.max(), freq=60, ax=ax4)
        im = m4.pcolormesh(HX, HY, ds2 - ds1, cmap='bwr', vmin=-20, vmax=20 )
