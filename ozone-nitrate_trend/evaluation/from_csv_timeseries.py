...
def main():
    ...
    for v in variables:
        ...
        df.plot(c='k',zorder=1, alpha=.5, ax=ax, label=site['site_name'])
        ...
        ax.plot( df.index, eval_DF['g'][1:], c="k", label='__nolegend__' )

        plt.legend(loc=0)
        plt.tight_layout()
        plt.savefig( f"plots/{site['save_name']}_{v}_lowess.png" )
        plt.close()
