...
def main():
    ...
    for nn, v in enumerate(variables):
        ...
        for n, rundir in enumerate(rundirs):
            print( rundir )
            ds0, lat, lon = get_data_as_xr(rundir, year='2015', lev=0, variable=v, version=versions[n])
            ds0 = np.mean( ds0, 0 )
        ...
        plot( f, axes, add_plottable, lat, lon, c=cmap, 
                cbar_label=[f'Surface {v} {units[nn]}','%'],
                labels=['Base','Shah et al. 2022','Shah et al. 2022 - Base','j100 / Base'],
                sname=f'Shah.{v}')
