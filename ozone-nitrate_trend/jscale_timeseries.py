...
def get_data_as_xr(rundir, year='', variable=False):
    ...
    if variable:
        if variable == 'NOx':
            ds = ( ds[f'SpeciesConc_NO'] + ds[f'SpeciesConc_NO2'] ).isel(lev=14) * float(d[variable]['scale'])
        else:
...
def main():
    ...
    rundirs = ['nitrate_photol_control','nitrate_photol_2015_all_scale-25'
                                        ,'nitrate_photol_2015_all_scale-50'
                                        ,'nitrate_photol_2015_all_scale-100'
                                        ,'nitrate_photol_2015_all_scale-1000']#,'nitrate_photol_CH4FIX']
    sname='j1000'

    variables=['O3']#,'NOx','HNO2']#,O3','CO']
    cs=['#1b9e77','#d95f02','#7570b3','#e7298a','#66a61e','#e6ab02']
    for v in variables:
