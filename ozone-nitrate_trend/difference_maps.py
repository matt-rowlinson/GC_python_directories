...
def main():
    ...
    rundirs = [#'nitrate_photol_2017_control', 
            'nitrate_photol_2015_all_scale-25',
                                         'nitrate_photol_2015_all_scale-25_100HONO',
                                         'nitrate_photol_2015_all_scale-25_100NO2' ]

    variables=['NOy']#,'O3','HNO2','NOx', 'NITs','NIT','CO','SO4','OH']
    for v in variables:
        print( v )
        ds=[]
        for n, rundir in enumerate(rundirs):
            print( n, rundir)
            ds0, lat, lon = get_data_as_xr(rundir, year='2019', lev=0, variable=v)
            ds0 = np.mean( ds0, 0 )
            ds.append( ds0 )
        print( 'Data loaded' ) 
        ## 2/3 ratios vs 100 HONO
        add_plottable=[]
        #add_plottable.append( np.array((18)) )
        f, ((ax1,ax2),(ax3,ax4)) = plt.subplots(2,2,figsize=(12,8))
        axes=[ax1,ax2,ax3,ax4]
    
        add_plottable.append( ds[0] ) 
        add_plottable.append( ds[1] )
        add_plottable.append( ds[1] - ds[0] )
        add_plottable.append( - ( 100 - ( ds[1] / ds[0] * 100 ))) 
        print( 'Makes first plot')
        plot( f, axes, add_plottable, lat, lon, c=cmap, 
                cbar_label=[f'{v} ppbv', f'{v} ppbv', f'{v} ppbv', '%'],
                labels=['67:33 HONO:NO2 Jscale=25', '100:0 HONO:NO2 Jscale=25', '100 HONO - 67:33','100 HONO / 67:33'],
                sname=f'all_HONO_vs_default_{v}')
        print( 'First plot complete')

        ## 2/3 ratios vs 100 NO2
        add_plottable=[]
        f, ((ax1,ax2),(ax3,ax4)) = plt.subplots(2,2,figsize=(12,8))
        axes=[ax1,ax2,ax3,ax4]

        add_plottable.append( ds[0] ) 
        add_plottable.append( ds[2] )
        add_plottable.append( ds[2] - ds[0] )
        add_plottable.append( - ( 100 - ( ds[2] / ds[0] * 100 ))) 
        print( 'Makes second plot')
        plot( f, axes, add_plottable, lat, lon, c=cmap, 
                cbar_label=['ppbv', 'ppbv', 'ppbv', '%'],
                labels=['67:33 HONO:NO2 Jscale=25', '0:100 HONO:NO2 Jscale=25', '100 NO2 - 67:33','100 NO2 / 67:33'],
                sname=f'all_NO2_vs_default_{v}')
