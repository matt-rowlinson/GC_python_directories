...
def EMEP_factors():
    ...
    for n, year in enumerate(years):
        ...
        df=df.transpose()
        df.index = ['Total European']#,'CH2O','EOH','Ethane','Propane','Propene','Ketones','ALK4','Benzene','Toluene','Xylene']
        plots_.append( df )
    return df
...
def NEI_factors():
    ...
    for n, year in enumerate(years):
        ...
        df=df.transpose()
        df.index = ['Total USA']#,'CH2O','EOH','Ethane','Propane','Propene','Ketones','ALK4','Benzene','Toluene','Xylene']
        return df
...
def main():
    ...
    df.columns=['CEDS Europe','Regional Europe (EMEP)','CEDS USA','Regional USA (NEI)']

    c = ['#66c2a5','#fc8d62','#66c2a5','#8da0cb']
    f,ax1=plt.subplots(1,1, figsize=(6,5.5))
    df.plot.bar(rot=0, ax=ax1, width=.6, color=c)

    ax1.set_ylabel('Total NMVOCs (Gg $yr^{-1}$)')
    ax1.legend()

    plt.tight_layout()
    plt.savefig('IGC10/plots/figure_3.png' )
    plt.close()
