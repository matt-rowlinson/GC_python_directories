...
def plot_by_region(all_regions, region_names, region_longnames, species='C2H6'):
    for y, region in enumerate(all_regions):
        ...
        axes[0].legend(['%s (%sN, %sE)' %(region[0][5], region[0][7], region[0][6])
                        ,'Control (Tzompa)', 'CEDS','CEDS + NEI/EMEP scaling', 'inc. Asia scaling' ])
...
if __name__=="__main__":
    ...
    dev2, gclat,gclon,lev, dev2_time = GC.get_gc_var(rundir='nei_emep_scaling',variable=variable,version=version,verbose=True, year='2017')
    dev3, gclat,gclon,lev, dev3_time = GC.get_gc_var(rundir='asia_scale_2.5',variable=variable,version=version,verbose=True, year='2017')
