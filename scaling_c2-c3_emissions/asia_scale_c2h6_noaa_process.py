...
def plot_individual_site(df, GC_data, site, group, year='2016', species='C2H6'):
    ...
    ax.legend([f'{site} ({y}N, {x}E)', 'Control (Tzompa)', 'CEDS + Geo', 'CEDS + Geo + NEI/EMEP scaling', 'inc. Asia scaling'])
...
def allocate_to_group(group, variables):
    print(len(variables))
    if group=='SH':
        shs.append([variables[0],variables[1],variables[2],variables[3],variables[4],variables[5],variables[6],variables[7],variables[8] ])
    elif group=='Polar':
        polars.append([variables[0],variables[1],variables[2],variables[3],variables[4],variables[5],variables[6],variables[7],variables[8] ])
    elif group=='NAM':
        nams.append([variables[0],variables[1],variables[2],variables[3],variables[4],variables[5],variables[6],variables[7],variables[8] ])
    elif group=='EUR':
        eurs.append([variables[0],variables[1],variables[2],variables[3],variables[4],variables[5],variables[6],variables[7],variables[8] ])
    elif group=='AFR':
        afrs.append([variables[0],variables[1],variables[2],variables[3],variables[4],variables[5],variables[6],variables[7],variables[8] ])
    elif group=='Asia':
        asias.append([variables[0],variables[1],variables[2],variables[3],variables[4],variables[5],variables[6],variables[7],variables[8] ])
    ...
    for y, region in enumerate(all_regions):
        ...
        for n in range(len(region)):
            ...
            axes[n].plot(region[n][0].resample('M').mean(), 'k-', label='%s (%sN, %sE)' 
                    %(region[n][-3], region[n][-1],region[n][-2]) )
            axes[n].legend()
            ...
            region[n][5].plot(ax=axes[n], linestyle='--', color='pink', x_compat=True, legend=False)

        axes[0].legend(['%s (%sN, %sE)' %(region[0][-3], region[0][-1], region[0][-2])
                        ,'Control (Tzompa)', 'Asia * 2.5','Asia * EMEP scaling', 'Asia * NEI scaling','Asia * average'  ])
        plt.suptitle(f'NOAA {species}: {region_longnames[y]}')
        plt.savefig(f'plots/Asia_Scale_{species}_noaa_{region_names[y]}-sites.png')
        plt.close()
...
if __name__=="__main__":
    ...
    dev, gclat,gclon,lev, dev_time = GC.get_gc_var(rundir='asia_scale_2.5',variable=variable,version=version, verbose=True, year='2017')
    dev2, gclat,gclon,lev, dev2_time = GC.get_gc_var(rundir='asia_scale_EMEP',variable=variable,version=version,verbose=True, year='2017')
    dev3, gclat,gclon,lev, dev3_time = GC.get_gc_var(rundir='asia_scale_NEI',variable=variable,version=version,verbose=True, year='2017')
    dev4, gclat,gclon,lev, dev3_time = GC.get_gc_var(rundir='asia_scale_average',variable=variable,version=version,verbose=True, year='2017')
    ...
    for infile in sorted(glob.glob('/users/mjr583/GC/Geo_ethane/ethane_datasets/*txt')):
        ...
        dev4_var = model_data_for_site(dev4, ref_time, lon, lat, alt_idx)


        ## Plot comparison at each individual site
        values = plot_individual_site( df, [ref_var, dev_var, dev2_var, dev3_var,dev4_var], site, group, species=variable)
        shs, polars, nams, eurs, asias, afrs, mapped = allocate_to_group(group, [values,ref_var,dev_var,dev2_var,dev3_var,dev4_var,site,x,y] )
