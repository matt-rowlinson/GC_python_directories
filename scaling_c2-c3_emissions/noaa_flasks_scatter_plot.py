...
if __name__=="__main__":
    ...
    dev2, gclat,gclon,lev, dev2_time = GC.get_gc_var(rundir='scale_all_vocs',variable=variable,version=version, verbose=False, year='2017')
    ...
    plt.ylabel(f'Model {d[variable]["abbr"]} (ppt)')
    plt.xlabel(f'Observations {d[variable]["abbr"]} (ppt)')
    
    plt.xlim(0., 7000)
    plt.ylim(0., 7000)

    #plt.xlim(0. , np.max([obs.max(), model1.max(), model2.max()]))
    #plt.ylim(0. , np.max([obs.max(), model1.max(), model2.max()]))
    ...
    plt.savefig(f'plots/all_flask_scatter_all_{variable}.png')
    plt.close()
