...
ax1.plot( df0[0].scale_all_vocs, color=colors[1], label='Scaled CEDS' )
...
ax2.plot( df0[1].scale_all_vocs, color=colors[1], label='Scaled CEDS' )
...
ax4.plot( df0[3].scale_all_vocs, color=colors[1], label='Scaled CEDS' )
...
df = df.loc['2017'].resample('D').mean()
ax5.plot( df0[4].ceds_only, c=colors[0], label='Base' )
ax5.plot( df0[4].scale_all_vocs, color=colors[1], label='Scaled CEDS' )
...
df = iB + nB + iP + nP
df = df.loc['2017'].resample('D').mean()
ax6.plot( df0[5].ceds_only, c=colors[0], label='Base' )
ax6.plot( df0[5].scale_all_vocs, color=colors[1], label='Scaled CEDS' )
ax6.plot( df, c='k', label='Observed' )
ax6.legend()
ax6.set_ylabel( 'ALK4 (pptv)' )

plt.tight_layout()
plt.savefig( 'paper_figures/figure_3b.png')   
plt.close()
