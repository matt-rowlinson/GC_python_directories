...
def main():
    ...
    for variable in variables:
        ...
        dev3, gclat, gclon, lev, dev3_time = GC.get_gc_var(rundir='asia-meic_scale',variable=variable, version=version, verbose=False, year='2016')
        ref = dev ; ref_time=dev_time
        ### Get observations for each flask site
        obs=np.arange(1) ; model1=np.arange(1) ; model2=np.arange(1) 
        model3=np.arange(1) ; model4=np.arange(1)
        ...
        RMSE_4, r2_4 = calc_stats( obs, model4 )
        print( obs.shape )
        print( model2.shape )
    ...
    Pdev3 = ax1.scatter( obs_[0], model4_[0], c='m', alpha=.2, zorder=3 )
    a3, _, _, _ = np.linalg.lstsq(obs_[0][:,np.newaxis], model4_[0], rcond=None)
    ax1.plot( obs_[0], a3 * obs_[0], '-', c='m', zorder=11, label='MEIC scale')
    ...
    Pdev3 = ax2.scatter( obs_[1], model4_[1], c='m', alpha=.2, zorder=1)
    a5,_,_,_ = np.linalg.lstsq(obs_[1][:,np.newaxis],model4_[1], rcond=None)
    ax2.plot(obs_[1], a5 * obs_[1], '-', c='m', zorder=11)
    ...
    RMSE_4 = calc_stats( obs_[0], model4_[0] )
    print( RMSE_2, RMSE_3, RMSE_4 )
    print( '\n' )  
    RMSE_2 = calc_stats( obs_[1], model2_[1] )
    RMSE_3 = calc_stats( obs_[1], model3_[1] )
    RMSE_4 = calc_stats( obs_[1], model4_[1] )
    print( RMSE_2, RMSE_3, RMSE_4 )

    plt.subplots_adjust(hspace=.2)
    plt.tight_layout()
    plt.savefig('plots/final_scatter.png')
    plt.close()
