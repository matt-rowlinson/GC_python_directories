...
def main():
    ...
    # ethane
    Pdev = ax1.scatter( obs_[0], model2_[0], c='g', alpha=.1, zorder=1)
    a2, _, _, _ = np.linalg.lstsq(obs_[0][:,np.newaxis], model2_[0], rcond=None)
    ax1.plot(obs_[0], a2 * obs_[0], '-', c='darkgreen', alpha=.3, zorder=10)
    ...
    # propane
    Pdev = ax2.scatter( obs_[1], model2_[1], c='g', alpha=.1, zorder=1)
    a2, _, _, _ = np.linalg.lstsq(obs_[1][:,np.newaxis], model2_[1], rcond=None)
    ax2.plot(obs_[1], a2 * obs_[1], '-', c='darkgreen', alpha=.3,zorder=10)
