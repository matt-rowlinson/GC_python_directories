...
ax1.plot(obs_[0], a2 * obs_[0], '-', c='darkgreen', zorder=10, label='Base')
...
ax1.set_xlim(0. , np.max([obs_[0].max(), model3_[0].max()]))
ax1.set_ylim(0. , np.max([obs_[0].max(), model3_[0].max()]))
...
ax2.plot(obs_[1], a2 * obs_[1], '-', c='darkgreen', zorder=10, label='Base')
ax2.set_ylabel(f'Model {d[variables[1]]["abbr"]} (ppt)')
ax2.set_xlabel(f'Observed {d[variables[1]]["abbr"]} (ppt)')

Pdev2 = ax2.scatter( obs_[1], model3_[1], c='darkorange', alpha=.3, zorder=2)
a2, _, _, _ = np.linalg.lstsq(obs_[1][:,np.newaxis],model3_[1], rcond=None)
ax2.plot(obs_[1], a2 * obs_[1], '-', c='darkorange', zorder=10, label='Scaled CEDS')
...
plt.savefig('paper_figures/figure_4.png')
plt.close()
