...
if __name__=="__main__":
    year = '2017'
    version='14.0.1' ; variable='ALK4'
    models = ['control']
    ### Get model data
    #ref, gclat,gclon,lev, ref_time = GC.get_gc_var(rundir='control',variable=variable,version=version, verbose=False, year='2017')
    dev, gclat,gclon,lev, dev_time = GC.get_gc_var(rundir='geo_2x25',variable=variable,version=version, verbose=False, year='2017')
    dev2, gclat,gclon,lev, dev2_time = GC.get_gc_var(rundir='all_2x25',variable=variable,version=version, verbose=False, year='2017')
    ...
    print( model2.shape )
    print( model3.shape )
    np.savetxt("ALK4_obs_v14.0.2x25.csv", obs, delimiter=",")
    np.savetxt("ALK4_GeoOnly_v14.0.2x25.csv", model2, delimiter=",")
    np.savetxt("ALK4_AllScale_v14.0.2x25.csv", model3, delimiter=",")
    sys.exit()
    fig, ax = plt.subplots(1,1,figsize=(6,6))  

    #Pref = ax.scatter( obs, model1, c='b', alpha=.3, zorder=2)
    #a1, _, _, _ = np.linalg.lstsq(obs[:,np.newaxis], model1, rcond=None)
    #ax.plot(obs, a1 * obs, '-', c='darkblue', zorder=10, 
    #        label= f'Tzompa (y={np.round(a1[0],2)}, RMSE={np.round(RMSE_1,2)}, R2={np.round(r2_1,2)})' )
    ...
    ax.plot(obs, a2 * obs, '-', c='darkgreen', zorder=10,
            label= f'GEOS-Chem base  (y={np.round(a2[0],2)}, RMSE={RMSE_2})')#, R2={r2_2})' )
     
    Pdev2 = ax.scatter( obs, model3, c='orange', alpha=.2, zorder=3)
    ...
    ax.plot(obs, a3 * obs, '-', c='darkorange', zorder=10, 
            label= f'CEDS emissions scaled  (y={np.round(a3[0],2)}, RMSE={RMSE_3})')#, R2={r2_3})' )
        

    #Pref.set_label( f'Tzompa (y={np.round(a1[0],2)}, RMSE={np.round(RMSE_1,2)}, R2={np.round(r2_1,2)})' )
    #Pdev.set_label( f'CEDS emissions  (y={np.round(a2[0],2)}, RMSE={np.round(RMSE_2,2)}, R2={np.round(r2_2,2)})' )
    #Pdev2.set_label( f'CEDS scaled  (y={np.round(a3[0],2)}, RMSE={np.round(RMSE_3,2)}, R2={np.round(r2_3,2)})' )

    plt.ylabel('Model  (ppt)')
    plt.xlabel('Observations (ppt)')

    plt.xlim(0. , np.max([np.quantile(obs,.995), np.quantile(model1, .995), np.quantile(model2, .995)]))
    plt.ylim(0. , np.max([np.quantile(obs,.995), np.quantile(model1, .995), np.quantile(model2, .995)]))
    ...
    plt.savefig(f'plots/BOTHall_flask_scatter_only_CEDS_{variable}.png')
    plt.close()
