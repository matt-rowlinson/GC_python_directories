...
def EMEP_factors():
    ...
    for n, year in enumerate(years):
        ...
        df = pd.DataFrame( {"CEDS VOCs" : np.array(np.squeeze(cf.values))  ,
                            ...
                            index = ef.index )
        df = df.transpose()
        df = df[['C2H6','C3H8']]#,'CH2O','EOH','C2H6','C3H8','PRPE','MEK','ALK4','BENZ','TOLU','XYLE']]
        df=df.transpose()
        df.index = ['Ethane','Propane']#,'CH2O','EOH','Ethane','Propane','Propene','Ketones','ALK4','Benzene','Toluene','Xylene']
        plots_.append( df )
    return df
    ...
    for n, year in enumerate(years):
        ...
        df = pd.DataFrame( {"CEDS" : np.array(np.squeeze(cf.values))  ,
                            "NEI" : np.array(np.squeeze(nf.values)) } , 
                             index = cf.index)
        df=df.transpose()
        df = df[['ethane','propane']]#,'methanal','alcohols','ethane','propane','propene','ketones','ALK4','benzene','toluene','xylene']]
        df=df.transpose()
        df.index = ['Ethane','Propane']#,'CH2O','EOH','Ethane','Propane','Propene','Ketones','ALK4','Benzene','Toluene','Xylene']
        return df
...
def main():
    ...
    df.columns=['CEDS Europe','Regional Europe','CEDS USA','Regional USA']

    c = ['#66c2a5','#fc8d62','#66c2a5','#8da0cb']
    f,ax1=plt.subplots(1,1, figsize=(6,5.5))
    df.plot.bar(rot=0, ax=ax1, width=.7, color=c)

    ax1.set_ylabel('Total emission (Gg $yr^{-1}$)')
    ax1.legend(ncol=2)

    plt.tight_layout()
    plt.savefig('IGC10/plots/figure_4.png' )
    plt.close()
