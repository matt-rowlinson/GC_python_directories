...
def allocate_to_group(group, variables):
    if group=='SH':
        shs.append([variables[0],variables[1],variables[2],variables[3],variables[4],variables[5],variables[6] ])
    elif group=='Polar':
    ...
    mapped.append([variables[-3],variables[-2],variables[-1]])
...
def plot_by_region(all_regions, region_names, region_longnames, species='C2H6'):
    for y, region in enumerate(all_regions):
        ...
        axes[0].legend(['%s (%sN, %sE)' %(region[0][-3], region[0][-1], region[0][-2])
                        ,'Control (Tzompa)', 'CEDS', 'EMEP/NEI/Asia scaled + geo','EMEP/NEI/Asia scaled - no geo'  ])
        plt.suptitle(f'NOAA {species}: {region_longnames[y]}')
        plt.savefig(f'plots/No_geo_{species}_noaa_{region_names[y]}-sites.png')
        plt.close()
...
if __name__=="__main__":
    ...
    dev2, gclat,gclon,lev, dev_time = GC.get_gc_var(rundir='asia_scale_2.5',variable=variable,version=version, verbose=True, year='2017')
    dev3, gclat,gclon,lev, dev2_time = GC.get_gc_var(rundir='asia_scale_no_geo',variable=variable,version=version,verbose=True, year='2017')
    ...
    for infile in sorted(glob.glob('/users/mjr583/GC/Geo_ethane/ethane_datasets/*txt')):
        ...
        shs, polars, nams, eurs, asias, afrs, mapped = allocate_to_group(group, [values,ref_var,dev_var,dev2_var,site,x,y] )
