...
def main():
    ...
    version = '14.0.1'
    species_=['NO']#SALA','SALC','Cl','SALCCL','SALACL']
    
    ## Makes simple global map plot
    import sys
    base_ = surface_data( 'bermuda_test', '20190702', version )
    dev_  = surface_data( 'bermuda_mask', '20190702', version )
    for species in species_:
        print( species )
        base = base_[f'SpeciesConc_{species}']
        dev  = dev_[f'SpeciesConc_{species}']
        cart_plot( [dev-base,- (100 - (dev/base*100))], sname=f'BMask.{species}_surfacemap')
