...
def process_obs_and_model_data( rundirs, ds_, var ):
    ...
    for infile in sorted( glob.glob( f'/users/mjr583/noaa_flask_data/{var}_datasets/*txt')):
        ...
        x_.append( df.longitude[0] )
        y_.append( df.latitude[0] )
        z_.append( df.altitude[0] )
    ...
    xyz = pd.DataFrame( {'Lon':x_,'Lat':y_, 'Alt':z_})
    xyz = xyz.drop_duplicates()
    xyz.to_csv('lat_lons_NOAA_VOCs.csv')
...
def main():
    ...
    scat_colours = ['forestgreen','darkorange']
    line_colours = ['forestgreen','darkorange']
    ...
    for ax, var,alpha in zip(axes, [ethane,propane,alk4],alphas):
        for n, rundir in enumerate(rundirs):
            ...
            ax.plot( x_fit, fit, c=line_colours[n], label=labels[n]+" (z=%.2f)" %a2 ) 
