...
def output_stats(var, obs, model, rundirs):
    ...
    for rundir, m in zip(rundirs,model):
        ...
        from scipy import stats
        slope, intercept, r_value, p_value, std_err = stats.linregress(o, m)
        ...
        bias = ((m-o) / o).mean() * 100
        if first:
            df = pd.DataFrame( { 'Slope':slope, 'intercept':intercept, 'r_value':r_value,'p_value':p_value,'std_err':std_err,
                            'Mean AbsError': absError.mean(), 'bias %': bias, 'NMB':nmb,'r$^2$':R2,'RMSE':RMSE},
                    index=[rundir]    )
            first=False
        else:
            d = pd.DataFrame( { 'Slope':slope, 'intercept':intercept, 'r_value':r_value,'p_value':p_value,'std_err':std_err,
                            'Mean AbsError': absError.mean(), 'bias %': bias, 'NMB':nmb,'r$^2$':R2,'RMSE':RMSE},
                    index=[rundir]    )
...
def to_table(var, df):
    ...
    plt.tight_layout
    plt.savefig(f'Output/noaa.{var}.stats.png', dpi=300, bbox_inches='tight')
    plt.close()
    return
...
def main():
    ...
    for n, v in zip( ['C2H6','C3H8','ALK4'],[ethane,propane,alk4]):
            v = output_stats(n, v.analysis_value, [v['geo_2x25'],v['all_2x25']], ['Base','Scaled emissions'] )
