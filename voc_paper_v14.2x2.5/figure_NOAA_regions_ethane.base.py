...
def main():
    ...
    EU_ethane  = process_obs_and_model_data(rundirs, ds_, 'C2H6', 'europe')
    NA_ethane  = process_obs_and_model_data(rundirs, ds_, 'C2H6', 'n-america')
    AS_ethane  = process_obs_and_model_data(rundirs, ds_, 'C2H6', 'asia')
    PA_ethane  = process_obs_and_model_data(rundirs, ds_, 'C2H6', 'pacific')
    AT_ethane  = process_obs_and_model_data(rundirs, ds_, 'C2H6', 'atlantic')
    ...
    NA_ethane  = NA_ethane[  NA_ethane.analysis_value  <= np.percentile( NA_ethane.analysis_value,  95 )]
    EU_ethane  = EU_ethane[  EU_ethane.analysis_value  <= np.percentile( EU_ethane.analysis_value,  99 )]
    AS_ethane  = AS_ethane[  AS_ethane.analysis_value  <= np.percentile( AS_ethane.analysis_value,  99 )]
    PA_ethane  = PA_ethane[  PA_ethane.analysis_value  <= np.percentile( PA_ethane.analysis_value,  99 )]
    AT_ethane  = AT_ethane[  AT_ethane.analysis_value  <= np.percentile( AT_ethane.analysis_value,  99 )]
    
    NA_ethane  = NA_ethane[  NA_ethane[rundirs[0]]  <= np.percentile( NA_ethane[rundirs[0]],  99 )]
    EU_ethane  = EU_ethane[  EU_ethane[rundirs[0]]  <= np.percentile( EU_ethane[rundirs[0]],  95 )]
    AS_ethane  = AS_ethane[  AS_ethane[rundirs[0]]  <= np.percentile( AS_ethane[rundirs[0]],  99 )]
    PA_ethane  = PA_ethane[  PA_ethane[rundirs[0]]  <= np.percentile( PA_ethane[rundirs[0]],  99 )]
    AT_ethane  = AT_ethane[  AT_ethane[rundirs[0]]  <= np.percentile( AT_ethane[rundirs[0]],  99 )]
    SH_ethane  = SH_ethane[  SH_ethane[rundirs[0]]  <= np.percentile( SH_ethane[rundirs[0]],  99 )]
    
    #print( NA_ethane.shape, EU_ethane.shape,AS_ethane.shape, PA_ethane.shape,AT_ethane.shape, SH_ethane.shape )
    ...
    ps = [NA_ethane, EU_ethane,AS_ethane, PA_ethane, AT_ethane, SH_ethane]
    for nn, ax, var,alpha in zip(range(6), axes, ps,alphas):
        for n, rundir in enumerate(rundirs):
            ...
            ax.plot( var.analysis_value, a2 * var.analysis_value, zorder=100, c=line_colours[n], label=labels[n]+" (m=%.2f)" %a2 )
            print( titles[nn], 'ODR= ', a2 )
        ...
        ax.set_xlim(0, 4000) ; ax.set_ylim(0, 4000)

        ax.set_ylabel(f'Model $C_2H_6$ (ppt)')
        ax.set_xlabel(f'Observed $C_2H_6$ (ppt)')

    plt.tight_layout()
    plt.savefig('plots/NOAA_regions_ethane.base.png')
    plt.close()
    ...
    for nn, ax, var,alpha in zip(range(6), axes, ps,alphas):
        for n, rundir in enumerate(rundirs):
            ...
            #print( rundir, titles[nn], '\nODR= ', a2 )
            rms = mean_squared_error(var.analysis_value, var[rundir], squared=False)
            #print( 'RMSE: ', rms )

            #nms = np.sqrt(((var.analysis_value -  var[rundir])**2).mean())
            #nms = 1 - (nms /  var.analysis_value.mean())
            #print( 'NMSE: ', nms )
        ...
        ax.set_xlim(60, 5000) ; ax.set_ylim(60, 5000)  
        print( nn ) 
        if nn==0 or nn==3:
    ...
    plt.tight_layout()
    plt.savefig('plots/NOAA_regions_ethane_log.base.png')
    plt.close()
