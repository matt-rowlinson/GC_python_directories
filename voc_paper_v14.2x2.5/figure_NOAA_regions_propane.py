...
def main():
    ...
    for nn, ax, var,alpha in zip(range(6), axes, ps,alphas):
        ...
        ax.set_ylabel(f'Model $C_3H_8$ (ppt)')
        ax.set_xlabel(f'Observed $C_3H_8$ (ppt)')
    ...
    for nn, ax, var,alpha in zip(range(6), axes, ps,alphas):
        for n, rundir in enumerate(rundirs):
            ...
            ax.plot( 10**x_fit, 10**fit, c=line_colours[n], zorder=200,
                        label=labels[n]+f" (NMB={nmb.round()}%)")
            #print( rundir, titles[nn], '\nODR= ', a2 )
            rms = mean_squared_error(var.analysis_value, var[rundir], squared=False)
            #print( var.analysis_value.min(), var.analysis_value.max() )
            #print( var[rundir].min() , var[rundir].max() )
    ...
    plt.tight_layout()
    plt.savefig('plots/NOAA_regions_propane_log.png')
    plt.close()
