...
def main():
    ...
    for site, ax, ax_ in zip(sites, axes, axes2):
        ...
        #print( df[form].max() )
        #print( d1[rundirs[1]].max())
        #print( d0[rundirs[0]].max())
        ...
        y0 = d0[rundirs[0]].reindex( df.dropna().index )
        ax_.scatter( df[form].dropna(),y0, s=4, c='darkorange', alpha=.2, zorder=100)
        x_fit, fit, a2 = get_odr( df[form].dropna(), y0 )
        ax_.plot( 10**x_fit, 10**fit, c='darkorange', zorder=200)

        y1 = d1[rundirs[1]].reindex( df.dropna().index )
        ax_.scatter( df[form].dropna(),y1, s=4, c='blue', alpha=.2, zorder=200)
        x_fit, fit, a2 = get_odr( df[form].dropna(), y1 )
        ...
        print(  (100 * np.sum(y0 - df[form]) / np.sum(df[form])).round(2) )
        print( (100 * np.sum(y1 - df[form]) / np.sum(df[form])).round(2) )

        nmb0_.append(  100 * np.sum(y0 - df[form]) / np.sum(df[form]) )
        nmb1_.append( 100 * np.sum( y1 - df[form]) / np.sum(df[form]) )
        ...
        '''if site=='Cape_Verde':
            ...
            alk4 = iB + nB + iP + nP
            alk4 = alk4['2016':'2017'].resample('D').mean()
            alk4 = alk4.replace( 0., np.nan )
            ax.plot( alk4, c='r', label='Observations - Merge' )
        '''
    print( np.array(nmb0_).mean() )
    print( np.array(nmb1_).mean() )
    
    print( np.array(nmb0_[1:]).mean() )
    print( np.array(nmb1_[1:]).mean() )
