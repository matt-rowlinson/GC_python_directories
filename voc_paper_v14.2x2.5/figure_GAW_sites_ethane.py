...
def main():
    ...
    gs  = matplotlib.gridspec.GridSpec(7, 4)
    ...
    ax14 = fig.add_subplot(  gs[6, 3])

    axes  = [ax1,ax2,ax3,ax4,ax5,ax6,ax7]
    axes2 = [ax8,ax9,ax10,ax11,ax12,ax13,ax14]

    colours = ['blue','darkorange'][::-1]
    sub=str.maketrans("0123456789", "₀₁₂₃₄₅₆₇₈₉")
    for site, ax, ax_ in zip(sites, axes, axes2):
        print( site )
        df, d0, coord = process_datasets( [ds0], rundirs, site, years, var, gcvar)
        ...
        ax.set_ylabel( f'{gcvar.translate(sub)} (pptv)', fontsize=13 )
        ax.set_ylim(bottom=170., top=4400)
        ax.set_yscale('log')
        ...
        rms = mean_squared_error( df[form].dropna(), y, squared=False)
        ax_.scatter( df[form].dropna(),y, s=4, c='darkorange', alpha=.2, zorder=100)#, label='Base (RMSE=%0.f ppt)' %rms)
        x_fit, fit, a2 = get_odr( df[form].dropna(), y )
        ax_.plot( 10**x_fit, 10**fit, c='darkorange', zorder=200, label='Base (RMSE=%0.f ppt)' %rms)

        ax_.legend(fancybox=True, facecolor='w', fontsize=8)

        ax_.set_xlim(left=170., right=4400) ; ax_.set_ylim(bottom=170., top=4400)
        ax_.set_xscale('log') ; ax_.set_yscale('log')
        
        ax_.set_ylabel( f'Model {gcvar.translate(sub)} (pptv)', fontsize=13 )

        if site != sites[-1]:
            plt.setp(ax.get_xticklabels(), visible=False)
            plt.setp(ax_.get_xticklabels(), visible=False)

    ax1.legend(loc="upper center", bbox_to_anchor=(0.5,1.2), ncol=3, fontsize=14)
    ax14.set_xlabel( f'Observed {gcvar.translate(sub)} (pptv)', fontsize=13 )
    myFmt = matplotlib.dates.DateFormatter('%m-%Y')
    ax.xaxis.set_major_formatter(myFmt)

    plt.tight_layout()
    plt.savefig( f'plots/figure_GAW_sites_{var}.base.png', dpi=300)   
    plt.close()
    ...
    for site, ax, ax_ in zip(sites, axes, axes2):
        ...
        ax.plot( d0[rundirs[0]], color=colours[0], label='Base')
        ax.plot( d1[rundirs[1]], color=colours[1], label='Scaled' )
        ax.plot( df[form], c='k', label='Observations' )
        ...
        y = d0[rundirs[0]].reindex( df.dropna().index )
        rms = mean_squared_error( df[form].dropna(), y, squared=False)
        nmb = 100 * np.sum( y - df[form].dropna()) / np.sum(df[form].dropna())
        print( site, 'Base', nmb.round(2), '%'  )

        ax_.scatter( df[form].dropna(),y, s=4, c='darkorange', alpha=.2, zorder=100)#, label='Base (RMSE=%0.f ppt)' %rms)

        print( df[form].dropna() )
        print( y )
        print( type( df[form].dropna() ), type( y ) )
        x_fit, fit, a2 = get_odr( df[form].dropna(), y )
        ax_.plot( 10**x_fit, 10**fit, c='darkorange', zorder=200, label='Base (RMSE=%0.f ppt)' %rms)
        ...
        rms = mean_squared_error( df[form].dropna(), y, squared=False)
        nmb = 100 * np.sum( y - df[form].dropna()) / np.sum(df[form].dropna())
        print( site, 'Scaled', nmb.round(2), '%'  )

        ax_.scatter( df[form].dropna(),y, s=4, c='blue', alpha=.2, zorder=200)#, label='Scaled (RMSE=%0.f ppt)' %rms)
        x_fit, fit, a2 = get_odr( df[form].dropna(), y )
        ax_.plot( 10**x_fit, 10**fit, c='b', zorder=200, label='Scaled (RMSE=%0.f ppt)' %rms)

        ax_.legend(fancybox=True, facecolor='w', fontsize=8)

        ax_.set_xlim(left=170., right=4400) ; ax_.set_ylim(bottom=170., top=4400)
        ax_.set_xscale('log') ; ax_.set_yscale('log')
        ax_.set_xticks([250,500,1000,2000,4000])
        ax_.get_xaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
        ax_.set_yticks([250,500,1000,2000,4000])
