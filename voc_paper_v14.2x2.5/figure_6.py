...
def main():
    ...
    labels = ['Ethane','Propane','Ethanol','Formaldehyde','Propene','MEK','ALK4','Benzene','Toluene','Xylene']
    ...
    for n in range(10):
        print( n )
        print( add_plottable[n].mean().values )
        ...
        ax.set_title(f'{panel_labels[n]} {labels[n]}', fontsize=12 )
        ax.coastlines() 
    
    plt.tight_layout()
    plt.savefig( f"plots/figure_6.png", dpi=200 )
    plt.close()
