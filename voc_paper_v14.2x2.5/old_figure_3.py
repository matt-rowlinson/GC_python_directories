...
def main():
    ...
    for variable in variables:
        ...
        dev, gclat,gclon,lev, dev_time = GC.get_gc_var(rundir='geo',variable=variable,version=version, verbose=False, year=year)
        dev2, gclat,gclon,lev, dev2_time = GC.get_gc_var(rundir='scale_all',variable=variable,version=version, verbose=False, year=year)
    ...
    #print( 'Base a2', a2 )
    ax1.plot(obs_[0], a2 * obs_[0], '-', c='darkgreen', zorder=10, label=f'Base (z={a2[0].round(2)})')
    ...
    #print( 'Dev a2', a2 )
    #ax1.plot( obs_[0], a2 * obs_[0], '-', c='darkorange', zorder=10, label=f'Scaled CEDS emissions (z={a2[0].round(2)})')
    ...
    print(  np.max([obs_[0].max(), model2_[0].max()]) )
    print( '!!!!!' )
    ax1.legend() 
