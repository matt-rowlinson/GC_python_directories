...
def main():
    ...
    EU_alk4  = process_obs_and_model_data(rundirs, ds_, 'ALK4', 'europe').drop_duplicates()
    NA_alk4  = process_obs_and_model_data(rundirs, ds_, 'ALK4', 'n-america').drop_duplicates()
    AS_alk4  = process_obs_and_model_data(rundirs, ds_, 'ALK4', 'asia').drop_duplicates()
    PA_alk4  = process_obs_and_model_data(rundirs, ds_, 'ALK4', 'pacific').drop_duplicates()
    AT_alk4  = process_obs_and_model_data(rundirs, ds_, 'ALK4', 'atlantic').drop_duplicates()
    SH_alk4  = process_obs_and_model_data(rundirs, ds_, 'ALK4', 'SH').drop_duplicates()
    
    ## Filter 99th percentile of data
    SH_alk4  = SH_alk4[  SH_alk4.analysis_value  <= np.percentile( SH_alk4.analysis_value,  99 )]
    NA_alk4  = NA_alk4[  NA_alk4.analysis_value  <= np.percentile( NA_alk4.analysis_value,  95 )]
    EU_alk4  = EU_alk4[  EU_alk4.analysis_value  <= np.percentile( EU_alk4.analysis_value,  99 )]
    AS_alk4  = AS_alk4[  AS_alk4.analysis_value  <= np.percentile( AS_alk4.analysis_value,  99 )]
    PA_alk4  = PA_alk4[  PA_alk4.analysis_value  <= np.percentile( PA_alk4.analysis_value,  99 )]
    AT_alk4  = AT_alk4[  AT_alk4.analysis_value  <= np.percentile( AT_alk4.analysis_value,  99 )]
    
    NA_alk4  = NA_alk4[  NA_alk4[rundirs[0]]  <= np.percentile( NA_alk4[rundirs[0]],  99 )]
    EU_alk4  = EU_alk4[  EU_alk4[rundirs[0]]  <= np.percentile( EU_alk4[rundirs[0]],  95 )]
    AS_alk4  = AS_alk4[  AS_alk4[rundirs[0]]  <= np.percentile( AS_alk4[rundirs[0]],  99 )]
    PA_alk4  = PA_alk4[  PA_alk4[rundirs[0]]  <= np.percentile( PA_alk4[rundirs[0]],  99 )]
    AT_alk4  = AT_alk4[  AT_alk4[rundirs[0]]  <= np.percentile( AT_alk4[rundirs[0]],  99 )]
    ...
    ps = [NA_alk4, EU_alk4,AS_alk4, PA_alk4, AT_alk4, SH_alk4]
    for nn, ax, var,alpha in zip(range(6), axes, ps,alphas):
        for n, rundir in enumerate(rundirs):
            ...
            ax.plot( var.analysis_value, a2 * var.analysis_value, zorder=100, c=line_colours[n], label=labels[n]+" (z=%.2f)" %a2 )
        ...
        ax.set_title(titles[nn],weight='bold')

        ax.set_ylabel(f'Model ALK4 (ppt)')
        ax.set_xlabel(f'Observed ALK4 (ppt)')

    plt.tight_layout()
    plt.savefig('plots/NOAA_regions_alk4.png')
    plt.close()
    ...
    for nn, ax, var,alpha in zip(range(6), axes, ps,alphas):
        for n, rundir in enumerate(rundirs):
            ...
            if first:
                df = pd.DataFrame( {'run':rundir, 'nmb':nmb.round(2)}, index=[titles[nn]] )
                first=False
            else:
                fd = pd.DataFrame( {'run':rundir, 'nmb':nmb.round(2)}, index=[titles[nn]] )
                df=pd.concat([df,fd])
            ...
            ax.plot( 10**x_fit, 10**fit, c=line_colours[n], zorder=200,
                        label=labels[n]+" (RMSE=%.0f ppt)" %rms)
            #x_fit, fit, a2 = get_odr( var.analysis_value, var[rundir] )
            #ax.plot( 10**x_fit, 10**fit, c=line_colours[n], zorder=200, label=labels[n])#, zorder=100)
        ...
        ax.set_xlim(0.1, 6000) ; ax.set_ylim(0.1, 6000)  
    ...
    plt.tight_layout()
    plt.savefig('plots/NOAA_regions_alk4_log.png')
    plt.close()

    print( df )

    print( df[df.run=='geo_2x25'].mean() )
    print( df[df.run=='all_2x25'].mean() )
