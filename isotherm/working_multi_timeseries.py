...
def get_data_as_xr(site, rundir, years, variable, version='13.1.2'):
    ...
    if type(years)==list:
        ...
        for year in years:
            ...
            files = find_file_list(path, ['SpeciesConc.'])
...
def plot_by_scale(ax, df, label, c='r', first=False):
    ...
    if first:
        try:
            pc = np.round( (z / df['1979'].values.mean()), 2)
            print( pc )
            sys.exit()
            ax.plot( df.index, df, c,label=f'{label}  (y={z} ppb decade$^{-1}$, {pc*100}%)' )
        except:
            pc = np.round( (z / df['1980'].values.mean()), 2)
            ax.plot( df.index, df, c,label=f'{label}  (y={z} ppb decade$^{-1}$, {pc*100}%)' )
    else:
        try:
            pc = (np.round( z / df['1979'].values.mean()), 2)
            ...
            try:
                ...
                pc = np.round( (z / df['2007'].values.mean()), 2)
        ax.plot( df.index, df, c,label=f'{label}  (y={z}, {pc*100}%)' )
...
def main():
    ...
    m = pd.concat([base, j100], axis=1).resample( 'M' ).mean()
    m.columns = ['Base','j100']
    ...
    merged=merged.resample('M').mean() * 1e9
    ...
    df = load_observations(site, 'O3')[:].resample("M").mean()
    ...
    if split_trend:
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(f'plots/{site["save_name"]}_trends_by_Jscaling.png')
        plt.close()
