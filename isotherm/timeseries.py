...
def find_model_output_for_site( rundirs, versions, v, years='2017'):
...
def main():
    ...
    versions = ['13.1.2']
    years='2015'
    cs = ['#377eb8','#984ea3','#e41a1c','#ff7f00']
    variable='NOx'

    add_times = find_model_output_for_site( rundirs, versions, variable, years=years)
    f, ax = plt.subplots(1,1,figsize=(10,4))
    
    df = load_observations(site, 'NOx')[:]#.resample("M").mean()
    df = df.groupby( df.index.dayofyear ).mean()
    
    f, ax = plt.subplots(1,1,figsize=(10,4))
    ax.plot( add_times[0].resample('D').mean().index, df[-334:], 'k', label='Observations')
    for n,nn in enumerate(add_times):
        ax.plot( nn.index, nn, cs[n], label=rundirs[n] )
    plt.legend()
    plt.tight_layout()
    plt.savefig('plots/TEST.NOx.png')
    plt.close()
