...
def site_data(ds1, ds, lat=16.9, lon=-24.9, lev=0):
    ...
    if type(lev)==int:
        data = ds1.isel( lon=x, lat=y, lev=lev)
    else:
...
def find_model_output_for_site(ds, rundirs, versions, v, years='2017', diurnal=False):
...
def main():
    ...
    sname='HONO_diurnal'
    variable='HNO2'#','ACET','ALD2']
    years='201802'
    cs=['#1b9e77','#e7298a','#d95f02','#7570b3','#e7298a','#66a61e','#e6ab02']
    vert=[] ; jvert=[]
    for rundir in rundirs:#variables:
        print( rundir )
        path=f'/mnt/lustre/users/mjr583/GC/{version}/rundirs/{rundir}/OutputDir/'
        ds0 = xr.open_mfdataset( find_file_list(path, [f'SpeciesConc.{years}']), combine='by_coords' )
        HONO = site_data( ds0['SpeciesConc_HNO2'], ds, lon=site['longitude'], lat=site['latitude'] ).mean(dim='time')
        vert.append( HONO * 1e12 )

        ds0 = xr.open_mfdataset( find_file_list(path, [f'JValues.{years}']), combine='by_coords' )
        jHONO = site_data( ds0['Jval_HNO2'], ds, lon=site['longitude'], lat=site['latitude'], lev=0 )
        print( jHONO.shape )
        jHONO = pd.DataFrame({site['save_name']:jHONO.values}, index=ds0['time'].values)
        print( jHONO )
        sys.exit()
    ...
    add_times = find_model_output_for_site(ds, rundirs, versions, variable, years=years, diurnal=True)
    
    f, (ax1,ax2) = plt.subplots(1,2, figsize=(10,6))
    ax3 = ax1.twinx()

    ax1.plot( add_times[0], label='Base')
    ax1.plot( add_times[1], label='dev.NIThv')

    ax2.plot( vert[0][:35], lev[:35] )
    ax2.plot( vert[1][:35], lev[:35] )
    
    import matplotlib.dates as mdates
    myFmt = mdates.DateFormatter('%H')
    ax1.xaxis.set_major_formatter(myFmt)

    ax1.set_ylabel( 'HONO (ppt)')
    ax1.set_xlabel( 'Hour')

    ax2.set_ylabel( 'Altitude (km)')
    ax2.set_xlabel( 'HONO (ppt)')
    plt.suptitle(site['site_name'], size=18, weight='roman')
    ax1.legend()
    plt.savefig(f'plots/HNO2_diurnal_{site["save_name"]}_Feb.png')
    plt.close()
