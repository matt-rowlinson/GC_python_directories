...
ref, gclat,gclon,lev, ref_time = GC.get_gc_var(rundir='fullchem_4x5_LVOCfalse',variable='ethane',version='GEOS-Chem', verbose=False)
print('Load development GC')
dev, gclat,gclon,lev, dev_time = GC.get_gc_var(rundir='geo_ethane_propane',variable='ethane',version='GEOS-Chem',verbose=False)
...
for y, region in enumerate(all_regions):
    ...
    for n in range(len(region)):
        ...
        axes[n].set_xlabel('Observed C2H6 (ppb)')
        axes[n].set_ylabel('Modelled C2H6 (ppb)')
    ...
    axes[0].legend(['Tzompa ethane', 'CEDS + geological ethane' ], bbox_to_anchor=(2., 1.11), ncol=3)
    plt.subplots_adjust(hspace=0.3) 
    plt.suptitle('NOAA C2H6: %s ' %region_longnames[y])
    plt.savefig('working_plots/c2h6_noaa_%s-sites.png' %region_names[y])
    plt.close()
