...
def surface_data(rundirs, versions=['13.1.2']):
    ...
    for n, rundir in enumerate(rundirs):
        ...
        print( (ds0['SpeciesConc_O3'] * 1e9).values.mean() ) 
    return add_
    ...
    for n in range(0,18):
        ...
        if first:
            ...
            im = all_plottable[n].plot.imshow( x='lon',y='lat', ax=ax, transform=ccrs.PlateCarree(), 
                    levels=levels, cmap='bwr', center=0., add_colorbar=False) 
        print( labels[n], all_plottable[n].values.mean() )
        #print( 'out')
        ax.coastlines()
...
def main():
    ...
    cart_plot( rundirs, base_, j100_, interval, plot_type='pc' , sname='18_panel_O3_diffs')
    sys.exit()
    cart_plot( rundirs, base_, j100_, interval, plot_type='abs', sname='18_panel_O3_diffs')
