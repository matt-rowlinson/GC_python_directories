...
def cart_plot(species, rundirs, base, jrun, sname=''):
    ...
    X,Y = np.meshgrid(all_plottable[0]['lon'],all_plottable[0]['lat'])
    cmaps=['viridis']#,'inferno','inferno_r','viridis_r','plasma','plasma_r','magma','magma_r','cividis','cividis_r']
    ...
    for cmap in cmaps:
        for n in range(len(all_plottable)):
            ...
            cm.set_over('w') #; cm.set_under('grey') 
        ...
        cbar = fig.colorbar(im, cax=cbar_ax,drawedges=True,  orientation='horizontal', extend='max')
        ...
        plt.savefig( f'plots/{sname}_{cmap}.png' , dpi=300)
        plt.close()
...
def main():
    ...
    for s in species:
        ...
        cart_plot( s, rundirs, base_, j100_, sname=f'3panel_diff-maps_{s}_pc')
