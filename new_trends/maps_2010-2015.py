...
def cart_plot(species, rundirs, base, jrun, plot_type='abs', sname=''):
    ...
    labels=["Base: 2015-2010", "j100: 2015-2010", "Diff: j100-Base"]
    for n in range(0,3):
        if plot_type=='abs':
            unit = f'{species} (ppbv)'
        elif plot_type=='pc':
        ...
        if n < 2:
            im0 = all_plottable[n].plot.imshow( x='lon',y='lat', ax=ax, transform=ccrs.PlateCarree(), 
                    levels=levels, cmap='bwr', add_colorbar=False)
            first=False
        else:
        ...
        plt.suptitle( f"{species}", fontsize=18)

    plt.subplots_adjust( bottom=.1) 
    cbar_ax = fig.add_axes([0.03, 0.15, 0.6, 0.02])
    cbar = fig.colorbar(im0, cax=cbar_ax, orientation='horizontal')
    cbar.ax.set_xlabel(f'{unit}', size=15)

    cbar_ax = fig.add_axes([0.67, 0.15, 0.3, 0.02])
...
def main():
    ...
    for s in species:
        ...
        cart_plot( s, rundirs, base_, j100_, plot_type='pc' , sname=f'EUmaps_2010-2015_{s}')
        cart_plot( s, rundirs, base_, j100_, plot_type='abs', sname=f'EUmaps_2010-2015_{s}')
