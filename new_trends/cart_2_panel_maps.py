#!/usr/bin/env python3
# -*- coding: utf-8 -*-
#SBATCH --job-name=jscale_map
#SBATCH --ntasks=1
#SBATCH --mem=16Gb
#SBATCH --partition=test
#SBATCH --time=00:30:00
#SBATCH --output=Logs/jscale_maps_%A.log
import sys
import os
import xarray as xr
...
def get_data_as_xr(rundir, year='', lev=0, version='13.1.2', variable=False):
    ...
    if variable=='OH':
        ...
        try:
            ...
            #print( find_file_list( path,  [f'SpeciesConc.{year}']) )
            #sys.exit()
...
def main():
    ...
    for nn, v in enumerate(variables):
        ...
        for n in range(1,7):
            ...
            ax.coastlines()
            ax.title.set_text( rundirs[n].replace("base_run_","") )

        plt.savefig( 'plots/O3_ccrs_TEST.png' )
        plt.close()
