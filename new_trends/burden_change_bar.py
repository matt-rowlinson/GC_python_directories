...
def main():
    ...
    yearly = pd.read_csv(f'csv/1980-2019_all_base_burden_{v}.csv', index_col=0)
    yearly.index = pd.to_datetime( yearly.index, format='%Y')

    base = pd.read_csv(f'csv/1750-2020_base_burden_{v}.csv', index_col=0).dropna()
    base.index = pd.to_datetime( base.index, format='%Y')

    j25 = pd.read_csv(f'csv/2015_j25_burden_{v}.csv', index_col=0).dropna()
    j25.index = pd.to_datetime( j25.index, format='%Y')

    j100 = pd.read_csv(f'csv/1750-2020_j100_burden_{v}.csv', index_col=0).dropna()
    j100.index = pd.to_datetime( j100.index, format='%Y') 

    viral = pd.read_csv(f'csv/1750-2015_viral_burden_{v}.csv', index_col=0).dropna()
    viral.index = pd.to_datetime( viral.index, format='%Y-%m-%d') 

    and22b = pd.read_csv(f'csv/2015_Ander22b_burden_{v}.csv', index_col=0).dropna()
    and22b.index = pd.to_datetime( and22b.index, format='%Y-%m-%d')
    ...
    ax.bar( 2015, j25['O3'], width=0.35, ls='--', linewidth=1.6,
                edgecolor='r', fill=False, zorder=0, label='NIThv at J25' )
    ax.bar( [1977,2015], viral['O3'], fill=False, zorder=1, width=.6, ls='--', linewidth=2, 
            edgecolor='#984ea3', label='Shah et al. 2022')
    ax.bar( 2015, and22b['O3'], fill=False, zorder=1, width=.6, ls='--', linewidth=2, 
            edgecolor='magenta', label='Andersen22b')

    x.insert(0,1977)
    x.insert(41,2020)
    #plt.ylim( bottom=220., top=425. )
    ...
    plt.legend(loc='upper center',bbox_to_anchor=(.6, 1.06),ncol=5)
    plt.ylabel( f"Tropospheric {v} burden (Tg yr$^{-1}$)" )
    plt.savefig(f'plots/{v}_burden_bar.png')
    plt.close()
