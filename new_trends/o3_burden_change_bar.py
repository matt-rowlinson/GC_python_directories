...
def main():
    ...
    yearly = pd.read_csv('csv/1980-2019_all_base_burden_o3.csv', index_col=0)
    yearly.index = pd.to_datetime( yearly.index, format='%Y')

    base = pd.read_csv('csv/1750-2020_base_burden_o3.csv', index_col=0).dropna()
    base.index = pd.to_datetime( base.index, format='%Y')

    j100 = pd.read_csv('csv/1750-2020_j100_burden_o3.csv', index_col=0).dropna()
    j100.index = pd.to_datetime( j100.index, format='%Y') 

    viral = pd.read_csv('csv/2015_viral_burden_o3.csv', index_col=0).dropna()
    ...
    ax.bar( 2015, viral['O3'], fill=False, zorder=1, width=.6, ls='--', linewidth=2, 
    ...
    plt.legend(loc='upper center',bbox_to_anchor=(.6, 1.06),ncol=4)
    plt.ylabel( "Tropospheric $O_3$ burden (ppbv yr$^{-1}$)" )
    plt.savefig(f'plots/o3_burden_bar.png')
    plt.close()
