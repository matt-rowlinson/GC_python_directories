...
def surface_data(rundirs, versions=['13.1.2']):
    ...
    for n, rundir in enumerate(rundirs):
        ...
        ds0 = ds0.mean('time')
        ds0 = ds0.isel( lev=0 )
        add_.append( (ds0['SpeciesConc_NIT']  +ds0['SpeciesConc_NITs'] +ds0['SpeciesConc_NITD1']+
                      ds0['SpeciesConc_NITD2']+ds0['SpeciesConc_NITD3']+ds0['SpeciesConc_NITD4'] )* 1e9 ) 
    return add_
...
def cart_plot(rundirs, base, jrun, interval, plot_type='abs', sname=''):
    ...
    for n in range(0,18):
        if plot_type=='abs':
            unit = r'$\Delta$ total NIT (ppbv/yr$^{-1}$)'
            if n < 11:
...
def main():
    ...
    cart_plot( rundirs, base_, j100_, interval, plot_type='pc', sname='18_panel_NIT_diffs')
    cart_plot( rundirs, base_, j100_, interval, plot_type='abs', sname='18_panel_NIT_diffs')
